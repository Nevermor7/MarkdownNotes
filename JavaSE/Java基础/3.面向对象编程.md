...
1. Java中允许将一个类A声明在另一个类B中，类A就是内部类，类B称为外部类。
...
2. 内部类的分类
   ...
   1. 成员内部类（静态、非静态）
      ...
      1. 作为外部类的成员
         ...
         ​	> 可以调用外部类的结构。`B.this.方法名()`

         ​	> 可以被`static`修饰。

         ​	> 可以被四种权限修饰符修饰。普通外部类只能被`public`或缺省修饰。

      2. 作为一个类

         ​	> 类内可以定义属性、方法、构造器等。

         ​	> 可以被`final`修饰，表示此类不能被继承。言外之意，不使用`final`是可以被继承的。

         ​	> 可以被`abstract`修饰。

   2. 局部内部类（方法内、构造器内、代码块内）
...
3. 常见问题
   ...
   3. 开发中局部内部类的使用。
   4. 成员内部类和局部内部类在编译后都会生成字节码文件。格式：成员内部类：`外部类名`+`$`+`内部类名.class`；局部内部类：`外部类名`+`$`+`出现序号`+`内部类名.class`。如：`Person.class`，`Person$Dog.class`，`Person$1AA.class`，`Person$2AA.class`。
   5. 由于外部类和内部类的字节码文件不是同一个，在局部内部类中调用外部类的局部变量时，实际上获取的到是该局部变量的副本，所以不能修改这个局部变量的值，JDK8之前如果出现这种调用，那么被调用的局部变量必须声明为`final`的，JDK8可以省略`final`关键字，但是依然不能修改值。
...
class Person {
    ...
    class Bird {
        ...
        public void sing() {
            ...
            eat(); // 调用外部类的非静态属性，完整写法：Person.this.eat();
        }
        public void display(String name) {
            // 成员内部类中调用外部类的结构
            System.out.print(name); // 形参name
            System.out.print(this.name); // 内部类Bird的属性name
            System.out.print(Person.this.name); // 外部类Person的属性name
        }
    }
    public void method() {
        // 局部内部类
...
public class InnerClassTest1 {
    ...
    public Comparable getComparable() {
        ...
        // 方式一
        class MyComparable implements Comparable { // 可能会这样用
            @Override
...
`abstract`可以修饰的结构：类、方法
  ...
  1. `abstract`修饰类：抽象类
     ...
     ​	> 抽象类可以继承非抽象的类。如：每个抽象类都默认继承`Object`类。

  2. `abstract`修饰方法：抽象方法

     ​	> 抽象方法只有方法的声明，没有方法体（大括号以及大括号之间的内容统称为方法体）。

     ​	> 由于抽象方法没有方法体，不能被调用。所以抽象方法只能声明在抽象类中（不能通过`对象.方法`调用），且不能是静态的（不能通过`类.方法`调用）。
...
IO流中的抽象类：InputStream / OutputStream / Reader / Writer
...
3. 如何定义接口：接口中的成员
   ...
   ​	④ 接口中的成员只能是public的（JDK9之前），JDK9之后可以有私有方法。

   ​			> 接口中，`default` == `public`，一般都用缺省权限。
...
- 包含的成员
  - 权限修饰符：JDK9之前，接口中的成员只能是`public`且默认为`public`的，而抽象类中可以有`private`的成员。（**接口无法封装，抽象类可以封装**）JDK9新特性，接口中可以定义`private`的方法。
  - 方法：JDK8之前，接口中只能有抽象方法，抽象类中既可以有抽象方法又可以有非抽象方法（包含静态和非静态）。JDK8新特性，接口中方法前加`default`关键字表示该方法为默认方法（非抽象），还可以直接定义静态方法。
  - 属性：接口中只有常量，抽象类中可以有成员属性（变量）。
  - 构造器：接口中没有构造器，抽象类中有构造器。
- 继承性
  - 一个类可以同时实现多个接口，但是只能同时继承一个抽象类。
  - 接口只能继承接口不能实现接口，一个接口可是继承多个接口；抽象类可以实现接口、继承抽象类、继承普通类（如`Object`）。
